...
// Mine
// type JoinReturn<Parts extends string[] = [], D extends string = '', _Result extends string = ''> =
//   Parts extends [ infer First extends string, ...infer Rest extends string[]]
//     ? JoinReturn<Rest, D, `${ _Result }${ First }${ Rest extends [] ? '' : D }`>
//     : _Result

// Remove _Result
type JoinReturn<Parts extends string[] = [], D extends string = ''> =
  Parts extends [ infer First extends string, ...infer Rest extends string[]]
    ? `${ First }${ Rest extends [] ? '' : D }${ JoinReturn<Rest, D> }`
    : ''

declare function join<D extends string>(delimiter: D): <Parts extends string[] = []>(...parts: Parts) => JoinReturn<Parts, D>
