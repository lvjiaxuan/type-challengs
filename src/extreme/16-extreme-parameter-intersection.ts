...
type IntersectParameters<X extends unknown[], Y extends unknown[], R extends unknown[] = []> = X extends []
  ? [...R, ...Y]
  : Y extends []
    ? [...R, ...X]
    : [X, Y] extends [ [(infer A)?, ...infer XT], [(infer B)?, ...infer YT]] // main
      ? '0' extends (keyof X) | (keyof Y) // Either of X or Y is not array
        ? IntersectParameters<
            XT, YT,
            [[], []] extends [X, Y]
              ? [...R, Intr<A, B>?] // For optional elements
              : [...R, Intr<A, B>]
          >
        : [...R, ...Intr<A, B>[]] // X and Y are array
      : never;
