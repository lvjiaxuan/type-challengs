...
type _PrimitiveTypesMap = { string: string; number: number; boolean: boolean }
type _UnionToIntersection<U> = (U extends unknown ? (arg: U) => void : never) extends (arg: infer I) => void ? I : never
type _GetRequiredFiled<T> = T extends { required: infer R extends string[] } ? R[number] : never
type _PickRequired<T, U> = [U] extends [never]
  ? Partial<T>
  : (Partial<Exclude<T, U>> & Pick<T, U & keyof T>) extends infer R
    ? { [K in keyof R]: R[K] }
    : never

type JSONSchema2TS< T, _Required extends string = _GetRequiredFiled<T>> =
  T extends { type: infer Type extends string }
    ? Type extends keyof _PrimitiveTypesMap
      ? T extends { enum: infer Enum extends unknown[] }
        ? Enum[number]
        : _PrimitiveTypesMap[Type]
      : T extends { type: 'object', properties?: infer Prop }
        ? (Prop extends object ? JSONSchema2TS<Prop, _Required> : Record<string, unknown>)
        : T extends { type: 'array', items?: infer Items }
          ? Items extends { type: infer ArrayType }
            ? ArrayType extends keyof _PrimitiveTypesMap
              ? _PrimitiveTypesMap[ArrayType][]
              : JSONSchema2TS<Items>[]
            : unknown[]
          : never
    : keyof T extends infer K
      ? [K] extends [never]
        ? {}
        : _PickRequired<_UnionToIntersection<K extends keyof T ? Record<K, JSONSchema2TS<T[K]>> : never>, _Required>
      : never
